...
model_1 = Sequential()
model_1.add(Flatten(input_shape=(state_size, hist_size)))
model_1.add(Dense(8))
model_1.add(Dense(8))
model_1.add(Dense(n_actions))

# Initialize value function
model_2 = Sequential()
model_2.add(Flatten(input_shape=(state_size, hist_size)))
model_2.add(Dense(8))
model_2.add(Dense(8))
model_2.add(Dense(n_actions))
...
model_1.compile(loss=huber_loss, optimizer=opt)

opt = RMSprop(lr=0.00025)
model_2.compile(loss=huber_loss, optimizer=opt)
...
while True:
	...
	while not done:
		...
		if np.random.rand() < e or counter < hist_size:
			...
			model = models[np.random.randint(0,2)]
		...
		if counter > min_replay_mem_size and not test and counter % update_freq == 0 and len(D) > (batch_size + hist_size):
			...
			model_a = models[choice]
			model_b = models[0 if choice == 1 else 1]
			...
			for i in batch_idxs:
				...
				if not d:
					...
					y = r + gamma * np.amax(model_a.predict(stack_)[0])
				...
				target_f = model_b.predict(stack)
				target_f[0][a] = y
				target_f = np.clip(target_f, -10, 10)
				ys.append(target_f)

			X = np.array(X).reshape(batch_size, state_size, hist_size)
			model_b.fit(X, np.array(ys).reshape(batch_size, n_actions), epochs=1, verbose=0)
